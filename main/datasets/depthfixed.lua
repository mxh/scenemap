...
local M = {}
local FixedDepthDataset = torch.class('scenemap.FixedDepthDataset', M)

function FixedDepthDataset:__init(opt, split)
    self.split       = split
    ...
    self.buffer.target = nil -- will be filled below
    ...
    local tmp_target = torch.Tensor(self.buffer_size, self.width, self.height, 3):float()
    fastReadFloat(self.storage_dir .. "/dep.bin", tmp_target)
    tmp_target = tmp_target:transpose(2, 4):transpose(3, 4)
    self.buffer.target = tmp_target:narrow(2, 1, 1)
...
function FixedDepthDataset:size()
    return math.ceil(self.buffer_size / self.batch_size)
end

function FixedDepthDataset:get_sample()
